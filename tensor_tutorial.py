...
mean_across_all_colors_img = img_t.mean(-3)  # 3rd from last is always color channel, that's the trick!
mean_across_all_colors_batch = img_t.mean(-3)

# Gotta unsqueeze the weights to be able to multiply the tensors
weights = weights.unsqueeze(-1).unsqueeze_(-1)  # Why unsqueeze_ 2nd time? This modifies in-place,
...
batch_weights = (batch_t * weights)  # why does this work? because by default 1s are inserted at the beginning of
